...
import static com.jijesoft.boh.core.plugin.util.Assertions.isTrue;
...
import com.jijesoft.boh.core.plugin.manager.store.MemoryPluginPersistentStateStore;
...
public class PluginsConfigurationBuilder
{
    ...
    /**
     * Builds a {@link com.jijesoft.boh.core.plugin.main.PluginsConfiguration} instance by processing the configuration that
     * was previously set, validating the input, and setting any defaults where not explicitly specified.
     * @return A valid {@link PluginsConfiguration} instance to pass to {@link JijePlugins}
     */
    public PluginsConfiguration build()
    {
        ...
        {
        	throw new IllegalArgumentException("Must be set the module Descriptor Factory");
        }

        if (osgiPersistentCache == null)
        {
            osgiPersistentCache = createTempDir("jijesoft-plugins-osgi-cache");
        }
    ...
    private File createTempDir(final String prefix)
    {
        try
        {
            final File directory = File.createTempFile(prefix, JijePlugins.TEMP_DIRECTORY_SUFFIX);
            directory.delete();
            directory.mkdir();
            return directory;
        }
