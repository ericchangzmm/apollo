...
$(document).ready(function(){
    ...
    function bindPortalAction(){
        ...
        $(".b-portal-title .b-sysList-1").live("click", function(){
            ...
            $.jsonAjax({
                url: "public/portal/getPortalPlugin",
        ...
        $(".b-portal-title .b-sysList-2").live("click", function(){
            var plug = this.parentNode.parentNode.parentNode.parentNode.parentNode;
            $.jsonAjax({
                url: "public/portal/deletePlugin",
                data: "id=" + plug.id,
                fn: function(data){
                    if (data.result) {
                        $(plug).remove();
    ...
    function execMessage(ms){
        $("#b-message-plug label").html(ms);
        $("#b-message-plug").slideDown(200);
        timerMessage = null;
        timerMessage = setTimeout("closeMessage()", 5000);
    }
     ...
     */
    function logout(){
        window.location.pathname = "/boh-mvc/web/login.html";
    }
    /***
     * 关于BOH
     */
        ...
        for (var i = 0; i < data.length; i++) {
            if (i >= 5) {
                if (i == 5) {
                    ...
                    li.innerHTML = "<img src='public/images/menu/menu-more-deful.png' dsrc='public/images/menu/menu-more-deful.png' alt='' ><div id='b-more-links'><span class='more-link'></span><dl></dl></div>";
                    firstMenu.append(li);
                    //绑定更多
                    li.onclick = function(){
                        $("#b-more-links").show();
    ...
    function bindTowMessage(data){
        ...
        if (data.list.length === 0) 
            div.innerHTML = "<span class='b-two-uninfo'>无新消息</span>";
        else {
            for (var i = 0; i < data.list.length; i++) {
                ...
                for (var m = 0; m < data.list[i].list.length; m++) {
                    ...
                    dl.innerHTML = "<dt><img src='" + data.list[i].list[m].icon + "'></dt><dd>玩的好的</dd>"
    ...
    function bindThreeEvent(){
        ...
        $(".b-Control-title .close").click(function(){
            $(this.parentNode.parentNode.parentNode).css({
                left: 232 + $("#b-right").width()
            });
        });
